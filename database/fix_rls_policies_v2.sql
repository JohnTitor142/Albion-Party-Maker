-- ============================================
-- CORRECTIF V2 : Suppression et Recréation des Policies RLS
-- Version sans permissions schema auth requises
-- ============================================
...
DROP POLICY IF EXISTS "Users can update profiles" ON users_profiles;
DROP POLICY IF EXISTS "Admins can delete profiles" ON users_profiles;
...
-- ============================================
-- 2. CRÉER UNE FONCTION HELPER DANS PUBLIC
-- ============================================
...
-- Mise à jour : Utilisateur peut modifier son propre profil OU admin
...
-- Toutes opérations : Admins seulement
CREATE POLICY "Admins can manage weapons insert"
    ON weapons FOR INSERT
    WITH CHECK (public.get_user_role(auth.uid()) = 'admin');

CREATE POLICY "Admins can manage weapons update"
    ON weapons FOR UPDATE
    USING (public.get_user_role(auth.uid()) = 'admin');

CREATE POLICY "Admins can manage weapons delete"
...
CREATE POLICY "ShotCallers can create compositions"
    ...
    WITH CHECK (
        public.get_user_role(auth.uid()) IN ('shotcaller', 'admin')
    );
...
-- Toutes opérations : Créateur de la composition ou Admin
CREATE POLICY "Composition owners can manage slots"
    ...
    USING (
        EXISTS (
            ...
            AND (c.created_by = auth.uid() OR public.get_user_role(auth.uid()) = 'admin')
        )
    );
...
CREATE POLICY "Users can view registrations"
    ...
    USING (
        ...
        EXISTS (
            ...
            AND (a.created_by = auth.uid() OR public.get_user_role(auth.uid()) = 'admin')
        )
    );
...
-- ============================================
-- FIN DU SCRIPT DE CORRECTION V2
-- ============================================

-- Vérifier que tout fonctionne
SELECT 'Policies RLS corrigées avec succès! (Version 2)' AS message;
